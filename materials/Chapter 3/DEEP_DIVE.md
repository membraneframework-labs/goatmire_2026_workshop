...
total duration of the audio that has passed before it.
...
* `:manual` - a _pull_ mode, in which the demand is handled by the element itself. Nothing
  arrives on a `:manual` input pad until the element demands it, and the element is notified about
...
In the happy path the whole cycle looks like this: `handle_demand/5` is called with, say, `20`,
the filter returns `demand: {:input, 20}`, then `handle_buffer/4` is called 20 times and each time
the filter sends the buffer on with `buffer: {:output, buffer}`. Once the downstream element wants
...
After the demand is requested, you can expect the demanded amount of data to
...
The plugins you'll need for all three tasks are already listed in `mix.exs`:
...
- make the output of `ColorInverter` end up in `Membrane.SDL.Player` (linking them directly
  won't work, if you have trouble figuring out why, take a look at the hints),
...
- keep the timestamps of the output stream continuous, i.e. shift the timestamps of the ad and of
  the resumed `:main` stream so that each segment starts right after the previous one ended,
- end the output stream when both inputs have ended (mind that by default an end of stream is forwarded immediately). If `:main` ends before the ad is being played, the switch should happen immediately afterwards, and the stream should end after the ad.
...
In code, the core of it looks roughly like this:

```elixir
  @impl true
  def handle_demand(:output, _size, :buffers, _ctx, state) do
    {[demand: {active_input_pad(state), 1}], state}
  end
...
same. Raising an error with a descriptive message on a mismatch is a nice touch,
but it's optional - it's a misconfiguration of the pipeline, not something the
...
Note that the default implementation of `handle_end_of_stream/3` forwards the end of stream to
the output right away, so as soon as you leave it unimplemented, the first input that ends will
end your output stream too, cutting off whatever should still be played.
</details>
