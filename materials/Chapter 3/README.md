...
4. shifts the timestamps of the ad and of the resumed `:main` so the output
   timeline stays continuous,
5. ends the output when both inputs have ended. If `:main` ends before the
...
- **Stream format.** Forward the first one received, assert the other pad's is
  the same.
- **End of stream.** `handle_end_of_stream/3` is called per pad and by default
  forwards the end of stream immediately. Implement it yourself, track which
  pads have ended and send `end_of_stream: :output` only once both of them have.
