...
There are two tasks, the second one built on top of the first. Start from the
`chapter-3-checkpoint` branch, the final solution is on `chapter-4-checkpoint`.

Add this dep to `mix.exs` and run `mix deps.get`:
...
- Serve `assets/` over HTTP in `run_pipeline.exs`, Boombox already brings the deps:
...
- The camera must send the same resolution as the ad, 480x270. In
  ...
  and add a note at the top of the file that it was modified.
- The input is live, so while the ad plays the main streams pile up in the
  toilets in front of the switchers' `:main` pads. The default capacity of 200
